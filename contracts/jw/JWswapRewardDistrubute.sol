...
contract JWswapRewardDistrubute is
    Initializable,
    ...
    UUPSUpgradeable
{
    ...
    function initialize(address _signer) public initializer {
        ...
        DOMAIN_SEPARATOR = keccak256(
            abi.encode(
                ...
                keccak256(bytes("JWswapRewardDistrubute")),
                keccak256(bytes("1")),
                chainId,
                address(this)
            )
    ...
    function withdrawReward(bytes memory data) public nonReentrant {
        ...
        require(user == msg.sender, "JWswapRewardDistrubute:INVALID_USER");
        require(nonce == nonces[msg.sender], "JWswapRewardDistrubute:INVALID_NONCE");
        require(block.timestamp <= deadline, "JWswapRewardDistrubute: TIME OUT");
        ...
        nonces[msg.sender]++;
        //send token
        require(balance(token) >= amount, "JWswapRewardDistrubute:INSUFFICIENT");
        if (token != address(0)) {
