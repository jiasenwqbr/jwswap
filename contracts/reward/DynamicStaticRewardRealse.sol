...
contract DynamicStaticRewardRealse is
    Initializable,
    ...
    UUPSUpgradeable
{
    ...
    function initialize(address _signer) public initializer {
        ...
        DOMAIN_SEPARATOR = keccak256(
            abi.encode(
                ...
                keccak256(bytes("DynamicStaticRewardRealse")),
                keccak256(bytes("1")),
                chainId,
                address(this)
            )
    ...
    // 用户领取
    function userClaim(bytes memory data) public nonReentrant {
        ...
        require(user == msg.sender, "DynamicStaticRewardRealse:INVALID_USER");
        require(nonce == nonces[msg.sender], "DynamicStaticRewardRealse:INVALID_NONCE");
        require(block.timestamp <= deadline, "DynamicStaticRewardRealse: TIME OUT");
        ...
        nonces[msg.sender]++;
        //send token
        require(balance(token) >= amount, "DynamicStaticRewardRealse:INSUFFICIENT");
        if (token != address(0)) {
            IERC20Upgradeable(token).safeTransfer(user, amount);
        } else {
            payable(user).transfer(amount);
        }
        emit UserClaim(msg.sender, token, amount, typeNum, accountId, block.timestamp);
    }
