...
contract NFTRewardDistribute is
    Initializable,
    ...
    UUPSUpgradeable
{
    ...
    function initialize(address _signer) public initializer {
        ...
        DOMAIN_SEPARATOR = keccak256(
            abi.encode(
                ...
                keccak256(bytes("NFTRewardDistribute")),
                keccak256(bytes("1")),
                chainId,
                address(this)
            )
    ...
    function userClaim(bytes memory data) public nonReentrant {
        ...
        require(user == msg.sender, "NFTRewardDistribute:INVALID_USER");
        require(nonce == nonces[msg.sender], "NFTRewardDistribute:INVALID_NONCE");
        require(block.timestamp <= deadline, "NFTRewardDistribute: TIME OUT");
        ...
        nonces[msg.sender]++;
        //send token
        require(balance(token) >= amount, "NFTRewardDistribute:INSUFFICIENT");
        if (token != address(0)) {
